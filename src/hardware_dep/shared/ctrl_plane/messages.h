...
*/
typedef enum p4_hello_elem_type {
	OFPHET_VERSIONBITMAP = 1 /* Bitmap of version supported. */
} p4_hello_elem_type_t;

/* Common header for all Hello Elements */
typedef struct p4_hello_elem_header {
	uint16_t type; /* One of OFPHET_*. */
	uint16_t length; /* Length in bytes of the element,
including this header, excluding padding. */
} p4_hello_elem_header_t;

/* Version bitmap Hello Element */
typedef struct p4_hello_elem_versionbitmap {
...
} p4_hello_elem_versionbitmap_t;

typedef struct p4_error {
	struct p4_header header;
	uint16_t error_code;
} p4_error_t;

typedef struct p4_success {
	struct p4_header header;
	uint32_t id;
} p4_success_t;
...
typedef enum p4_field_match_type {
	P4_FMT_EXACT = 0,
	P4_FMT_TERNARY = 1,
	P4_FMT_LPM = 2,
	P4_FMT_RANGE = 3,
	P4_FMT_VALID = 4
} p4_field_match_type_t;

typedef struct p4_field_match_desc {
	char name[P4_MAX_FIELD_NAME_LEN];
	uint8_t type;
} p4_field_match_desc_t;

typedef enum p4_support_timeout {
	P4_STO_FALSE = 0,
	P4_STO_TRUE = 1
} p4_support_timeout_t;

typedef enum p4_action_type {
	P4_AT_ACTION = 0,
	P4_AT_ACTION_PROFILE = 1
} p4_action_type_t;

typedef struct p4_table_action_desc {
	uint8_t type;
	char name[P4_MAX_ACTION_NAME_LEN];
} p4_table_action_desc_t;

typedef struct p4_table_definition {
	struct p4_header header;
...
typedef struct p4_field_match_valid {
    ...
    char bitmap[MAX_FIELD_LENGTH];
} p4_field_match_valid_t;

typedef struct p4_add_table_entry {
	struct p4_header header;
	char table_name[P4_MAX_TABLE_NAME_LEN];
	uint8_t read_size;
