...
for part_idx, (table, table_info) in enumerate(all_table_infos):
    ...
    if table.key_bit_size == 0:
        ...
        #[         bool is_hit = true; // empty key in table ${table.name}
        ...
        if len(noaction_names) == 0:
            #[         bool is_hit = true; // lookup on table ${table.name} cannot miss
        elif len(noaction_names) == 1:
            #[         bool is_hit = entry->id != action_${noaction_names[0]};
        else:
        ...
        #{         #ifdef T4P4S_DEBUG
        #[             ${tname}_apply_show_hit_with_key(is_hit, entry  KEYTXTPARAM_IN, STDPARAMS_IN);
        #}         #endif

        #{         #ifdef T4P4S_STATS
        #[             t4p4s_stats_global.T4STAT(table,hit,$tname) = is_hit || t4p4s_stats_global.T4STAT(table,hit,$tname);
        #[             t4p4s_stats_global.T4STAT(table,miss,$tname) = !is_hit || t4p4s_stats_global.T4STAT(table,miss,$tname);
        #[             t4p4s_stats_per_packet.T4STAT(table,hit,$tname) = is_hit || t4p4s_stats_per_packet.T4STAT(table,hit,$tname);
        #[             t4p4s_stats_per_packet.T4STAT(table,miss,$tname) = !is_hit || t4p4s_stats_per_packet.T4STAT(table,miss,$tname);
        #}         #endif

    if len(table.direct_meters + table.direct_counters) > 0:
        #[         if (likely(is_hit))    ${tname}_apply_smems(STDPARAMS_IN);
    ...
    if len(table.actions) == 1:
        ...
        #[         return (apply_result_t) { is_hit, action_${ao.name} };
    else:
        #{         switch (entry->id) {
...
for part_idx, (ctl, idx, comp) in enumerate(all_ctl_stages):
    multi_idx = part_idx % part_count
    #{ #if T4P4S_MULTI_IDX == ${multi_idx}
